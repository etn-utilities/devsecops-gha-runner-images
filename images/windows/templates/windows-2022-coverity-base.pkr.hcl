...
variable "container_name" {
  type = string 
  default = "${env("CONTAINER_NAME")}"
}

variable "blob_file_name" {
  type    = string
  default = "COVERITY_CLIENT_EXE_NAME"
}

# variable "storage_account_url" {
#   type    = string
#   default = "${env("CoverityAnalysisUrlName")}"
# }
...
source "azure-arm" "image" {
 ...
 shared_image_gallery_destination {
       ...
       image_version                     =  "2024.8.11"                                            #"2024.7.24"                                         
 }
...
build {
  ...
  #     "Start-Process 'C:\\coverity-client.exe' -ArgumentList '/S' -Wait",
  #      # Verify the installation directory and add to PATH
  #     "$env:CoverityInstallDir = \"C:\\Program Files\\Coverity\"",
  #     "if (Test-Path $env:CoverityInstallDir) {",
  #     "  [System.Environment]::SetEnvironmentVariable('PATH', [System.Environment]::GetEnvironmentVariable('PATH', [System.EnvironmentVariableTarget]::Machine) + \";\" + $env:CoverityInstallDir, [System.EnvironmentVariableTarget]::Machine)",
  #     "}",
  #      # Verify Coverity installation
  #     "$coverityPath = (Get-Command coverity -ErrorAction SilentlyContinue).Path",
  #     "if (-not $coverityPath) {",
  #     "  Write-Error 'Coverity installation failed or Coverity client not found in PATH'",
  #     "  exit 1",
  #     "} else {",
  #     "  Write-Output 'Coverity client installed successfully and found in PATH'",
  #     "}"
  #   ]
  # }
  ...
  provisioner "powershell" {
    inline = [
      ...
      "& $env:SystemRoot\\System32\\Sysprep\\Sysprep.exe /oobe /generalize /mode:vm /quiet /quit | Out-Null",
