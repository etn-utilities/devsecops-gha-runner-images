function Install-Binary {
    ...
    for ($retries = 20; $retries -gt 0; $retries--) {
        try {
            ...
            Write-Host "Package downloaded and verified in $attemptSeconds seconds"
            break
        } catch {
            $attemptSeconds = [math]::Round(($(Get-Date) - $attemptStartTime).TotalSeconds, 2)
        ...
        if ($retries -eq 0) {
            ...
            throw "Package download and verification failed after $totalSeconds seconds"
        }
...
function Install-BinaryWithoutRetry {
    <#
    .SYNOPSIS
        A function to install binaries from either a URL or a local path.
